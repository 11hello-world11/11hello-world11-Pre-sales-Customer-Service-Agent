...
import { handleQuery, handleExecute, handleListTables, handleDescribeTable, handleShowStatement, handleExplain } from './handlers.js';
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            switch (request.params.name) {
                ...
                case 'query':
                    return await handleQuery(this.pool, request.params.arguments);
                case 'execute':
                    return await handleExecute(this.pool, request.params.arguments);
                case 'list_tables':
                    return await handleListTables(this.pool);
                case 'describe_table':
                    return await handleDescribeTable(this.pool, request.params.arguments);
                case 'show_statement':
                    return await handleShowStatement(this.pool, request.params.arguments);
                case 'explain':
                    return await handleExplain(this.pool, request.params.arguments);
                default:
    ...
    async handleConnectDb(args) {
        ...
        if (args.url) {
            try {
                ...
                throw new McpError(ErrorCode.InvalidParams, `Invalid MySQL URL: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        }
        ...
        if (!newConfig && !this.config) {
            throw new McpError(ErrorCode.InvalidParams, 'No database configuration provided');
        }
